...
def seed_questionnaire_version(apps, schema_editor):
    QuestionnaireVersion = apps.get_model("onboarding", "QuestionnaireVersion")

    # Skip if there is already an active version
    if QuestionnaireVersion.objects.filter(is_active=True).exists():
        return
    ...
    existing_version = QuestionnaireVersion.objects.filter(version=1).first()
    if existing_version:
        existing_version.is_active = True
        existing_version.questionnaire_structure = questionnaire_structure
        existing_version.save(update_fields=["is_active", "questionnaire_structure"])
        return
...
def unseed_questionnaire_version(apps, schema_editor):
    QuestionnaireVersion = apps.get_model("onboarding", "QuestionnaireVersion")
    QuestionnaireVersion.objects.filter(version=1).delete()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(seed_questionnaire_version, unseed_questionnaire_version),
    ]
