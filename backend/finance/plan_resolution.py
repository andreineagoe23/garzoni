...
def primary_price_id_from_subscription(obj: Any) -> Optional[str]:
    ...
    if not data:
        return None
    first = data[0]
    price_obj = getattr(first, "price", None) if not isinstance(first, dict) else first.get("price")
    if isinstance(price_obj, str):
        return price_obj
    if price_obj is None:
        return None
    return (
        getattr(price_obj, "id", None) if not isinstance(price_obj, dict) else price_obj.get("id")
    )
