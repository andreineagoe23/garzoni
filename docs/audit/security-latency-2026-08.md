...
2. **The public API had a second representation.** DRF's browsable renderer answered
   `Accept: text/html` with 38 KB of HTML _and_ a `Set-Cookie`, on the exact prefix Cloudflare
...
### 1.3 The latent hole that was there — content negotiation _(fixed)_
...
### 2.1 The complete API map was served to anonymous callers _(fixed)_
...
### 2.2 `www.garzoni.app/api/*` is an infinite redirect loop _(fixed)_
...
2. That strip happens _before_ rewrites, so the `/api/:path*/` rule (which matches the slashed
...
### 2.3 Mobile shipped axios 1.14.0 _(fixed)_
...
Residual, low: both accept a 20–25 MB multipart upload _before_ the quota check rejects it, and the
...
| 2   | **Both calls are then made again.** `index.tsx` uses raw uncached `apiClient.get`, then the dashboard refetches the same two endpoints via `useQuery` — and the questionnaire one sets `staleTime: 0, refetchOnMount: true`, so it _always_ refetches. | `mobile/app/(tabs)/index.tsx:187-192, 204-210`    | 2 redundant round trips per open                      |
...
**Not changed, by decision.** The sequencing matters: the deployed web bundle points _at_ that
...
Related, and the reason host-header poisoning is _not_ a finding: `USE_X_FORWARDED_HOST = True`
...
_different allowed_ host (the Railway domain) could be injected into a cached `image_url`.
...
- ✅ **Boot calls no longer serial.** The local plan-cache read now happens _first_, since it
...
- ✅ **`staleTime: 0, refetchOnMount: true` removed** from the questionnaire query on _both_ the
...
- Mobile token handling is correct: SecureStore only, never AsyncStorage
  ...
  financial figures _off_ a legacy plaintext AsyncStorage key (`mobile/src/state/cfoProfile.ts:34-53`).
