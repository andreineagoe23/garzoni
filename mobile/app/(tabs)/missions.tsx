...
function MissionsInner() {
  ...
  const previousMissionsRef = useRef(new Map<string | number, string | undefined>());
  ...
  useEffect(() => {
    if (missionsQuery.data && typeof missionsQuery.data.can_swap === "boolean") {
  ...
  const getLessonRequirement = useCallback(
    (mission: Mission) => {
      const ref = mission.goal_reference as { required_lessons?: number } | undefined;
  ...
  const purposeStatement = useCallback(
    (mission: Mission) => {
      if (mission.purpose_statement?.trim()) return mission.purpose_statement.trim();
  ...
  const missionsRemaining = dailyMissions.filter((m) => m.status !== "completed").length;
  ...
  const handleSavingsSubmit = useCallback(async () => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: queryKeys.savingsBalance() });
  ...
  return (
    ...
    >
      ...
      <GlassCard padding="md" style={{ marginBottom: spacing.lg }}>
        <View style={styles.summaryRow}>
          <View style={styles.summaryLeft}>
            ...
            {isOffline ? (
              <Text style={styles.offline}>{t("missions.summary.offline")}</Text>
            ) : null}
            {adaptiveHints ? (
              <Text style={[styles.hintChip, { color: c.accent, borderColor: `${c.accent}44` }]}>
      ...
      {errorMessages.length > 0 ? (
        <GlassCard
          padding="md"
          style={{ marginBottom: spacing.md, borderColor: `${c.error}66`, backgroundColor: c.errorBg }}
        >
            ...
            <GlassCard padding="lg" style={{ marginBottom: spacing.lg }}>
              <View style={styles.wrapRow}>
                <View style={styles.wrapMascot}>
                  ...
                  <Text style={[styles.wrapMascotCap, { color: c.textMuted }]}>
              ...
              <View style={[styles.wrapCta, { borderColor: `${c.accent}66` }]}>
...
const styles = StyleSheet.create({
  ...
  summaryMain: { fontSize: typography.md, fontWeight: "800", marginTop: spacing.xs },
  summaryXp: { fontSize: typography.sm, marginTop: 4 },
  offline: { marginTop: spacing.sm, fontSize: typography.xs, color: "#d97706", fontWeight: "600" },
  hintChip: {
  ...
  streakWrap: { flexDirection: "row", flexWrap: "wrap", gap: spacing.sm, marginTop: spacing.md },
  streakPill: {
    paddingHorizontal: spacing.md,
    paddingVertical: 6,
    borderRadius: 999,
  ...
  grid: { gap: 0 },
  sectionTitle: { fontSize: typography.lg, fontWeight: "800", marginBottom: spacing.md, marginTop: spacing.sm },
  wrapRow: { flexDirection: "column", gap: spacing.md },
  wrapMascot: { alignItems: "center" },
  wrapMascotCap: { fontSize: typography.xs, fontWeight: "600", marginTop: spacing.xs, textAlign: "center" },
  wrapKicker: { fontSize: 10, fontWeight: "800", letterSpacing: 1, textTransform: "uppercase" },
  wrapTitle: { fontSize: typography.lg, fontWeight: "800", marginTop: spacing.xs },
