...
function ResultRow({
  ...
  return (
    <View style={resStyles.row}>
      <Text style={[resStyles.label, { color: colors.textMuted }]}>
        {label}
      </Text>
      <Text
        style={[
  ...
  return (
    <View style={preStyles.row}>
      {presets.map((p) => (
        ...
        >
          <Text style={[preStyles.label, { color: colors.textMuted }]}>
            {p.label}
          </Text>
        </Pressable>
      ))}
...
function Divider({ color }: { color: string }) {
  return (
    <View
      style={{ height: 1, backgroundColor: color, marginVertical: spacing.xs }}
    />
  );
}
    ...
    <View style={{ gap: spacing.md }}>
      <PresetRow
        ...
        presets={[
          {
            label: "$10k @ 7% / 10yr",
            values: { principal: "10000", rate: "7", years: "10", freq: "12" },
          },
          {
            label: "$1k @ 5% / 5yr",
            values: { principal: "1000", rate: "5", years: "5", freq: "12" },
          },
        ]}
        onApply={(v) => {
      ...
      {result && (
        <>
          ...
          <ResultRow
            label="Future Value"
            value={fmtCurrency(result.fv)}
            colors={c}
            highlight
          />
          <ResultRow
            label="Interest Earned"
            value={fmtCurrency(result.interest)}
            colors={c}
          />
        </>
      )}
    ...
    <View style={{ gap: spacing.md }}>
      <PresetRow
        ...
        presets={[
          {
            label: "$20k car / 60mo",
            values: { amount: "20000", rate: "6", months: "60" },
          },
          {
            label: "$200k home / 360mo",
            values: { amount: "200000", rate: "4.5", months: "360" },
          },
        ]}
        onApply={(v) => {
          setAmount(v.amount);
    ...
    <View style={{ gap: spacing.md }}>
      <PresetRow
        ...
        presets={[
          {
            label: "$5k → $7k",
            values: { invested: "5000", returned: "7000" },
          },
          {
            label: "$10k → $13k",
            values: { invested: "10000", returned: "13000" },
          },
        ]}
        onApply={(v) => {
          setInvested(v.invested);
          setReturned(v.returned);
        }}
      />
      <InputRow
    ...
    <View style={{ gap: spacing.md }}>
      ...
      {result && (
        <>
          ...
          <ResultRow
            label="Needs %"
            value={`${fmt(result.needsPct, 1)}%`}
            colors={c}
          />
          <ResultRow
            label="Wants %"
            value={`${fmt(result.wantsPct, 1)}%`}
            colors={c}
          />
          <ResultRow
            label="Savings %"
            value={`${fmt(result.savingsPct, 1)}%`}
            colors={c}
          />
          <ResultRow
            label="Leftover"
