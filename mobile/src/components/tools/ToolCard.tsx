...
export default function ToolCard({ tool, onPress, locked }: Props) {
  ...
  return (
    <Pressable onPress={onPress} disabled={locked} style={({ pressed }) => [{ opacity: pressed ? 0.92 : 1 }]}>
      <GlassCard padding="md" style={locked ? { opacity: 0.55 } : undefined}>
        <View style={styles.row}>
          <View style={styles.textCol}>
            <Text style={[styles.title, { color: c.text }]}>{tool.title}</Text>
            <Text style={[styles.sub, { color: c.textMuted }]}>{tool.subtitle}</Text>
            {tool.plusOnly ? (
              <Text style={[styles.badge, { color: c.accent }]}>Plus / Pro</Text>
            ) : null}
          </View>
        </View>
      </GlassCard>
