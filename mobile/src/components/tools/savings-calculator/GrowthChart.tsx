...
export function GrowthChart({ data, goalAmount }: Props) {
  ...
  const sy = (v: number) => PAD.top + innerH - (maxY > 0 ? (v / maxY) * innerH : 0);
  ...
  return (
    <View style={[styles.card, { backgroundColor: c.surface, borderColor: c.border }, shadows.sm]}>
      <Text style={[styles.title, { color: c.textMuted }]}>Growth Projection</Text>
      <Svg width={chartWidth} height={CHART_HEIGHT}>
        ...
        {yTicks.map((t, i) => (
          <React.Fragment key={i}>
            <Line
              x1={PAD.left} y1={t.y} x2={chartWidth - PAD.right} y2={t.y}
              stroke={c.border} strokeWidth={1} strokeDasharray="4,4"
            />
            <SvgText x={PAD.left - 4} y={t.y + 4} fontSize={9} fill={c.textFaint} textAnchor="end">
        ...
        {goalY != null && goalAmount > 0 && (
          <>
            <Line
              x1={PAD.left} y1={goalY} x2={chartWidth - PAD.right} y2={goalY}
              stroke={c.accent} strokeWidth={1.5} strokeDasharray="6,3"
            />
            <SvgText x={chartWidth - PAD.right} y={goalY - 4} fontSize={9} fill={c.accent} textAnchor="end">
              Goal
            </SvgText>
          </>
        )}
        ...
        <Polyline
          points={points} fill="none" stroke={c.primary}
          strokeWidth={2} strokeLinejoin="round" strokeLinecap="round"
        />

        {xTicks.map((d, i) => (
          <SvgText key={i} x={sx(d.month)} y={CHART_HEIGHT - 4} fontSize={9} fill={c.textFaint} textAnchor="middle">
            {d.month >= 12 ? `yr ${(d.month / 12).toFixed(0)}` : `mo ${d.month}`}
          </SvgText>
        ))}
      </Svg>
    </View>
