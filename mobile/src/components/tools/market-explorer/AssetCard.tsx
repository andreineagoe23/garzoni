  ...
  return (
    ...
    >
      <View style={styles.left}>
        <Text style={[styles.ticker, { color: c.text }]}>{asset.ticker.toUpperCase()}</Text>
        <Text style={[styles.name, { color: c.textMuted }]} numberOfLines={1}>
          {asset.name}
        </Text>
      </View>
      <View style={styles.right}>
        <Text style={[styles.price, { color: c.text }]}>{formatPrice(asset.price)}</Text>
...
const styles = StyleSheet.create({
  ...
  changeBadge: { borderRadius: radius.full, paddingHorizontal: spacing.sm, paddingVertical: 2 },
