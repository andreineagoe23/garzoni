...
const Questionnaire = () => {
  ...
  useEffect(() => {
    ...
    const billing_interval = params.get("billing_interval");
    if (plan_id) {
      setSelectedPlan({ plan_id, billing_interval });
    }
  }, [location.search]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!selectedPlan.plan_id) {
        ...
        setSubmitFeedback("Please choose a plan before continuing.");
      ...
      if (status === 503) {
        setSubmitFeedback(
          "Plans are not available right now. Please return to the plans page."
        );
        navigate("/subscriptions");
        return;
      }
      ...
      <div className="mx-auto flex max-w-4xl flex-col gap-8">
        <header className="space-y-3">
          ...
          {selectedPlan.plan_id && (
            <p className="inline-flex w-fit items-center rounded-full bg-[color:var(--primary,#2563eb)]/10 px-3 py-1 text-xs font-semibold text-[color:var(--primary,#2563eb)]">
              Selected plan: {selectedPlan.plan_id}
